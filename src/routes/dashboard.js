...
router.get("/:chave", async (req, res) => {
    ...
    try {
        ...
        const categoriasComNomes = await Promise.all(categoriasDespesas.map(async (item) => {
        ...
        const metodosPagamentoComNomes = await Promise.all(metodosPagamento.map(async (item) => {
        ...
        const corretorMaiorComissao = corretoresArray.length > 0
            ? corretoresArray.reduce((prev, current) => prev.totalComissao > current.totalComissao ? prev : current)
            : null;
        // Corretor com maior receita
        const corretorMaiorReceita = corretoresArray.length > 0
            ? corretoresArray.reduce((prev, current) => prev.totalReceita > current.totalReceita ? prev : current)
            : null;

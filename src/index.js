...
app.post("/dev/clientes", async (req, res) => {
    const { id, chave, razaoSocial, nomeFantasia, contato, cpf, cnpj, endereco, numero, bairro, cidade, estado, cep, inscMunicipal, atividade, email, fone } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
...
app.post("/contratos", async (req, res) => {
    const { chave, clienteId, dataEmissao, dataVencimento, programaId, numInsercoes, valor, corretorId, comissao, diaVencimento, formaPagamentoId, status, descritivo } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
...
app.post("/dev/contratos", async (req, res) => {
    const { id, chave, clienteId, dataEmissao, dataVencimento, programaId, numInsercoes, valor, corretorId, comissao, diaVencimento, formaPagamentoId, status, descritivo } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
...
app.post("/faturamento", async (req, res) => {
    const { chave, clienteId, contratoId, programaId, corretoresId, comissao, dataEmissao, dataVencimento, dataPagamento, valor, formaPagamentoId, descritivo } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
...
app.post("/dev/faturamento", async (req, res) => {
    const { id, chave, clienteId, contratoId, programaId, corretoresId, comissao, dataEmissao, dataVencimento, dataPagamento, valor, formaPagamentoId, descritivo } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
...
app.put('/faturamento/:id/pagamento', async (req, res) => {
    ...
    const { dataPagamento, formaPagamentoId } = req.body;
    if (!dataPagamento) {
...
app.post("/dev/corretores", async (req, res) => {
    const { id, chave, nome, endereco, email, fone, dataAdmissao } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
...
app.post("/forma-pagamento", async (req, res) => {
    const { chave, formaPagamento } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
...
app.post("/dev/forma-pagamento", async (req, res) => {
    const { id, chave, formaPagamento } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
...
app.post("/dev/programacao", async (req, res) => {
    const { id, chave, programa, horaInicio, horaFim, apresentador, diasApresentacao, valorPatrocinio, estilo } = req.body;
    if (!chave) {
        res.status(400).json({ error: "Chave de acesso não fornecida" });
        return;
    }
